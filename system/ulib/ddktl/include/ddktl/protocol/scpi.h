...
private:
    static zx_status_t ScpiGetSensor(void* ctx, const char* name, uint32_t* sensor_value) {
        return static_cast<D*>(ctx)->ScpiGetSensor(name, sensor_value);
    }
...
public:
    ...
    zx_status_t ScpiGetSensor(const char* name, uint32_t* sensor_value) {
        return ops_->get_sensor(ctx_, name, sensor_value);
    }
    zx_status_t ScpiGetSensorValue(uint32_t sensor_id, uint32_t* sensor_value) {
